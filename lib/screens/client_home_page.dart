...
//   Future<void> _fetchClientDetails(String email) async {
//     try {
//       // Dummy API call (replace with your actual API URL)
//       var response = await http.get(Uri.parse('http://10.0.2.2:3001/api/get-details/get-provider-data'));
...
//     super.initState();
//     _email = widget.email;
//     // Call the function to fetch client details
//     _fetchClientDetails(_email);
//   }
...
class _ClientProfile extends State<ClientProfile> {
  ...
  Future<void> _fetchClientDetails(String email) async {
    try {
      final url = Uri.parse('http://10.0.2.2:3001/api/get-details/get-client-data');
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _address1 = data['data']['address1'] ?? "";
          _address2 = data['data']['address2'] ?? "";
  ...
  // Update client details using API
  Future<void> _updateClientDetails() async {
    try {
      final url = Uri.parse('http://10.0.2.2:3001/api/modify/update-client');
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to update client details');
      }
    } catch (error) {
  ...
  void initState() {
    super.initState();
    _email = widget.email;
    _fetchClientDetails(_email); // Fetch details on initialization
  }
  ...
  void _submitForm() async {
    if (_clientProfile.currentState!.validate()) {
      ...
      await _updateClientDetails(); // Save updated details to server
